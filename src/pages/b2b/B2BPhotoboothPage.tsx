...
export default function B2BPhotoboothPage() {
  const { user } = useAuth();
  ...
  const fetchFrames = async () => {
    ...
    try {
      ...
      // CHỈ lấy những Frame dành cho Photobooth (có prefix [PB])
      setFrames(allFrames.filter((f: ArFrame) => f.frameName.startsWith('[PB]')));
    } catch (err) {
      console.error('Failed to fetch Photobooth layouts:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    // Tự động gán prefix [PB] để backend lưu chung vào bảng ArFrame nhưng phân biệt được
    formData.append('frameName', `[PB] ${frameName}`);
  ...
  const handleDelete = async (frameId: string) => {
    if (!window.confirm('Bạn có chắc chắn muốn xóa Layout này?')) return;
    try {
  ...
  // Hàm loại bỏ chữ [PB] để hiển thị cho đẹp
  const displayFrameName = (name: string) => {
    return name.startsWith('[PB] ') ? name.substring(5) : name;
  };
  ...
  return (
    <B2BLayout>
      <div className="max-w-6xl mx-auto space-y-8">
        <div>
          <h2 className="text-3xl font-black text-white mb-2 tracking-tight">Quản lý Layout Photobooth</h2>
          <p className="text-gray-400">Tải lên và quản lý các mẫu Layout để in ảnh Photobooth (dải 3 ảnh, 4 ảnh, ảnh vuông...).</p>
        </div>

        {/* Upload Section */}
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
          <h3 className="text-lg font-bold text-white mb-4">Tải lên Layout mới</h3>
          <form onSubmit={handleUpload} className="flex flex-col sm:flex-row gap-4 items-end">
            <div className="flex-1 w-full">
              <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Tên Layout</label>
              <input
                type="text"
                value={frameName}
                onChange={e => setFrameName(e.target.value)}
                placeholder="VD: Layout Dải 3 Ảnh Giáng Sinh"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {loading ? (
            <div className="col-span-full text-center py-12 text-gray-400">Đang tải danh sách Layout...</div>
          ) : frames.length === 0 ? (
            <div className="col-span-full text-center py-12 bg-white/5 border border-white/10 rounded-2xl border-dashed">
              <p className="text-gray-400">Chưa có Layout nào. Hãy tải lên Layout Photobooth đầu tiên của bạn!</p>
            </div>
          ) : (
            frames.map(frame => (
              <div key={frame.id} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden flex flex-col group">
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    <h4 className="text-white font-bold truncate" title={displayFrameName(frame.frameName)}>{displayFrameName(frame.frameName)}</h4>
